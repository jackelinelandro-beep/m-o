(() => {
  ...
  $("#calendar-download").addEventListener("click", () => {
    const calendar = [
      "BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Mercedes y Oscar//Boda//ES", "CALSCALE:GREGORIAN",
      "BEGIN:VEVENT", "UID:boda-mercedes-oscar-20260829@bejar-unlocked", "DTSTAMP:20260711T120000Z",
      "DTSTART:20260829T100000Z", "DTEND:20260829T180000Z", "SUMMARY:Boda de Mercedes y Oscar",
  ...
  photoForm.addEventListener("submit", async event => {
    ...
    const file = photoForm.elements.photo.files[0];
    if (!file) return;
    if (!/^image\/(jpeg|png|webp)$/.test(file.type) || file.size > 8 * 1024 * 1024) return setStatus("#photo-status", "Usa una imagen JPG, PNG o WebP de máximo 8 MB.", "error");
    ...
    const path = `public/${randomId()}.${extension}`;
    setSubmitting(photoForm, true);
    setStatus("#photo-status", "Subiendo la foto…");
    try {
      const upload = await db.storage.from("wedding-photos").upload(path, file, { cacheControl: "31536000", upsert: false, contentType: file.type });
